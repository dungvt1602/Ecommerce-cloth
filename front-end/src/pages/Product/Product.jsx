...
const Product = () => {
  ...
  const {products , currency, addToCart, cardItems}  = useContext(ShopContext);
    ...
    <div className='border-t-2 border-gray-200 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
        ...
        <div className='flex-1 '>
            ...
            <div className="flex gap-2">
              {
                productData.sizes.map((item,index) => (
                  <button onClick={() => setSize(item)} className={`text-sm border-1 py-2 px-4 bg-gray-100 ${size === item ? "border-orange-400" : "" } `} key={index}>{item}</button>
                ))
              }
            </div>
          </div>
          ...
          active:bg-gray-700 '>ADD TO CART</button>
          <hr className='mt-8 sm:w-4/5' />
          <div className="text-sm text-gray-500 mt-5 flex flex-col gap-1"></div>
          <p>100% sản phẩm từ thiên nhiên.</p>
          <p>Sản phẩm có thể trả bằng thẻ.</p>
          <p>Trả và hoàn lại tiền dễ dàng trong vòng 7 ngày.</p>
        </div>
      ...
      <div className="mt-20">
        <div className="flex">
          <p className="border text-sm py-3 px-5">Description</p>
          <p className="border text-sm py-3 px-5">Review (122)</p>
        </div>

        <div className='flex flex-col gap-4 border text-sm text-gray-500 px-5 py-2'>
      ...
      {/* REALTED PRODUCT SORT LIKE PRODUCT  */}
